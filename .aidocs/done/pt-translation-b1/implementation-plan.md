...
| TASK-10 | TG_CHANNEL_PT config + helper | `pipeline/config.py`, `pipeline/telegram.py` (add `send_photo_pt`/`send_text_pt` or unified arg), `tests/test_telegram.py` (clear-fail test) | ~5k | — | `TG_CHANNEL_PT_USERNAME = "pastelka_pt"` set. Empty `TG_CHANNEL_PT_ID` raises a `RuntimeError` with operator-actionable text. |
